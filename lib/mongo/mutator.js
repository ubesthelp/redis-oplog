...
export default class Mutator {
  static insert(Originals, data, _config) {
    ...
    try {
      ...
      if (!this._skipRedis && config.pushToRedis && (!this._weakDiffFields || _.difference(_.keys(doc), this._weakDiffFields).length)) {
  ...
  static update(Originals, selector, modifier, _config, callback) {
    ...
    try {
      ...
      const result = config.skipDB ? 0 : Originals.update.call(this, updateSelector, modifier, config)
      ...
      if (this._skipDiff || config.skipDiff) {
        ...
        docs.forEach((before, index) => {
          ...
          if (diff) {
            if (!config.skipSave || this.hasCache(after._id)) this.setCache(after)
      ...
      if (!this._weakDiffFields || _.difference(topLevelFields, this._weakDiffFields).length) {
        dispatchUpdate(
          config.optimistic,
          this._name,
          config._channels,
  ...
  static handleUpsert(
    ...
    try {
      ...
      if (result.insertedId) {
        ...
        if (typeof selector == 'string') {
          if (modifier.$set || modifier.$setOnInsert) doc = { ...modifier.$set, ...modifier.$setOnInsert, _id:result.insertedId }
          else doc = { ...modifier, _id:result.insertedId }
        }
        else if (typeof selector == 'object') {
          if (modifier.$set || modifier.$setOnInsert) doc = { ...selector, ...modifier.$set, ...modifier.$setOnInsert, _id:result.insertedId }
        ...
        if (docIds.length === 0 || result.numberAffected !== docIds.length) {
          ...
          if (this._skipDiff || config.skipDiff) {
            ...
            docs.forEach((before, index) => {
              ...
              const diff = DeepDiff(before, after, true)
              if (diff) {
                if (!config.skipSave || this.hasCache(doc._id)) this.setCache(after)
                docs[index] = _.pick(after, [...diff[0], '_id'])
              }
              // nothing to change -- get rid of it
              else delete docs[index]
